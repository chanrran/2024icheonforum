...
if uploaded is not None:
    ...
    with col1:
        ...
        with btn1:
            if st.button("회사별 건수"):
                display_option = st.radio("표/그래프 선택", ["표", "그래프"], key="company")
        ...
        with btn2:
            if st.button("카테고리별 건수"):
                display_option = st.radio("표/그래프 선택", ["표", "그래프"], key="category")
        ...
        with btn3:
            if st.button("수준별 건수"):
                display_option = st.radio("표/그래프 선택", ["표", "그래프"], key="level")
    ...
    if st.button("Wordcloud 만들기"):
        ...
        wordcloud = WordCloud(width=800, height=400, background_color='white', font_path='NanumGothic').generate(text)
        fig, ax = plt.subplots()
